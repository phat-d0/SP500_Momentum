...
def backtest(historical_data, cash=100000, allocation_pct=0.5, trade_log_file="trades_log.csv"):
    portfolio = []
    pnl_history = []
    cash_balance = cash
    ...
    for date in dates:
        ...
        # Simulate shorting top 10 and buying bottom 10
        prices = {s: historical_data[s].loc[date, 'open'] for s in historical_data if date in historical_data[s].index}
        place_trades(portfolio, bottom_movers, allocation, prices, 'buy')  # Buy bottom 10 movers
        place_trades(portfolio, top_movers, allocation, prices, 'sell')  # Short top 10 movers
...
def analyze_performance(pnl_history, cash):
    ...
    final_cash = cash + final_pnl
    initial_cash = cash
    total_pnl_percentage = (final_pnl / initial_cash) * 100

    print("\nPortfolio Performance Summary")
    print("----------------------------")
    print(f"Beginning Portfolio Value: ${initial_cash:,.2f}")
    ...
    print(f"Sharpe Ratio:             {calculate_sharpe_ratio(pnl_history):.4f}")
